...
namespace IRC_Client
{
    ...
    public partial class IRC
    {
        ...
        private string ServerNameFromPing = "";

        private static readonly byte endLineChar = Encoding.UTF8.GetBytes("\n")[0];

        public IRC(string Channel)//, int Channel_ID)
        {
        ...
        private void ConnectComplete(IAsyncResult result)
        {
            try
            {
                ...
                string pw = "oauth:" + TwitchApi.Token;

                SendRawMessage("PASS {0}", pw);

                SendRawMessage("NICK {0}", TwitchApi.Name);

                SendRawMessage("USER {0} hostname servername :{1}", TwitchApi.Name, TwitchApi.Name);
